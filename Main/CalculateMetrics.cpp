...
int CalculateMetrics::countByProfessionAndDecade(const std::vector<Human> &humans, const std::string &professionFilter, std::map<std::string, int> &output)
{
    for (const Human &human : humans)
    {
        if (human.getProfessions().find(professionFilter) != std::string::npos)
        {
            std::string birthDate = human.getBirthDate();
            if (birthDate.size() >= 4)
            {
                std::string yearStr = birthDate.substr(0, 4);
                try
                {
                    int year = std::stoi(yearStr);
                    int decade = (year / 10) * 10;
                    std::string decadeLabel = std::to_string(decade);
                    output[decadeLabel]++;
                }
                catch (...)
                {
